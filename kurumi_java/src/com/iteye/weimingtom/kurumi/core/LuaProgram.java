...
import com.iteye.weimingtom.kurumi.model.SMain;
...
public class LuaProgram {
	...
	private static LuaStateObject globalL = null;

	private static CharPtr progname = CharPtr.toCharPtr(LuaConf.LUA_PROGNAME);

	private static void lstop(LuaStateObject L, LuaDebugState ar) {
		LuaDebug.lua_sethook(L, null, 0, 0);
		LuaAuxLib.luaL_error(L, CharPtr.toCharPtr("interrupted!"));
	}

	public static class lstop_delegate implements LuaHook {
	...
	private static int report(LuaStateObject L, int status) {
	...
	private static int traceback(LuaStateObject L) {
	...
	private static int docall(LuaStateObject L, int narg, int clear) {
		int status;
	...
	private static int getargs(LuaStateObject L, String[] argv, int n) {
		int narg;
		int i;
	...
	private static int dofile(LuaStateObject L, CharPtr name) {
		int status = (LuaAuxLib.luaL_loadfile(L, name) != 0) || (docall(L, 0, 1) != 0) ? 1 : 0;
		return report(L, status);
	}

	private static int dostring(LuaStateObject L, CharPtr s, CharPtr name) {
		int status = (LuaAuxLib.luaL_loadbuffer(L, s, LuaConf.strlen(s), name) != 0) || (docall(L, 0, 1) != 0) ? 1 : 0; //(uint)
		return report(L, status);
	}

	private static int dolibrary(LuaStateObject L, CharPtr name) {
		Lua.lua_getglobal(L, CharPtr.toCharPtr("require"));
		LuaAPI.lua_pushstring(L, name);
		return report(L, docall(L, 1, 1));
	}

	private static CharPtr get_prompt(LuaStateObject L, int firstline) {
		CharPtr p;
	...
	private static int incomplete(LuaStateObject L, int status) {
		if (status == Lua.LUA_ERRSYNTAX) {
			int[] lmsg = new int[1]; //uint
			CharPtr msg = LuaAPI.lua_tolstring(L, -1, lmsg); //out
	...
	private static int pushline(LuaStateObject L, int firstline) {
	...
	private static int loadline(LuaStateObject L) {
		int status;
	...
	private static void dotty(LuaStateObject L) {
		int status;
		...
		while ((status = loadline(L)) != -1) {
			...
			if (status == 0 && LuaAPI.lua_gettop(L) > 0) {
				...
				if (LuaAPI.lua_pcall(L, LuaAPI.lua_gettop(L) - 1, 0, 0) != 0) {
					l_message(progname, LuaAPI.lua_pushfstring(L, CharPtr.toCharPtr("error calling " + LuaConf.LUA_QL("print").toString() + " (%s)"), Lua.lua_tostring(L, -1)));
				}
			}
		}
	...
	private static int handle_script(LuaStateObject L, String[] argv, int n) {
		int status;
		CharPtr fname;
	...
	private static int runargs(LuaStateObject L, String[] argv, int n) {
		int i;
		for (i = 1; i < n; i++) {
			if (argv[i] == null) {
				continue;
	...
	private static int handle_luainit(LuaStateObject L) {
	...
	private static int pmain(LuaStateObject L) {
	...
	public static class pmain_delegate implements LuaCFunction {
		public final int exec(LuaStateObject L) {
			return pmain(L);
		}
	}

	public static class traceback_delegate implements LuaCFunction {
		public final int exec(LuaStateObject L) {
			return traceback(L);
		}
	}
	...
	public static int MainLua(String[] args) {
		...
		int status;
		SMain s = new SMain();
		LuaStateObject L = Lua.lua_open(); // create state 
		if (L == null) {
