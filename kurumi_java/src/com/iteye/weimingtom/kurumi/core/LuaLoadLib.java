...
public class LuaLoadLib {
	...
	public static void setprogdir(LuaStateObject L) {
...
//		 
	...
	public static Object ll_load(LuaStateObject L, CharPtr path) {
		//(void)path;  /* to avoid warnings */
		Lua.lua_pushliteral(L, CharPtr.toCharPtr(DLMSG));
		return null;
	}

	public static LuaCFunction ll_sym(LuaStateObject L, Object lib, CharPtr sym) {
	...
	private static Object ll_register(LuaStateObject L, CharPtr path) {
...
//		 ** handle
//		 
	private static int gctm(LuaStateObject L) {
	...
	private static int ll_loadfunc(LuaStateObject L, CharPtr path, CharPtr sym) {
		...
		if (reg == null) {
			...
			LuaCFunction f = ll_sym(L, reg, sym);
			if (f == null) {
	...
	private static int ll_loadlib(LuaStateObject L) {
...
//		 
	...
	private static CharPtr pushnexttemplate(LuaStateObject L, CharPtr path) {
		CharPtr l;
	...
	private static CharPtr findfile(LuaStateObject L, CharPtr name, CharPtr pname) {
		CharPtr path;
	...
	private static void loaderror(LuaStateObject L, CharPtr filename) {
	...
	private static int loader_Lua(LuaStateObject L) {
		CharPtr filename;
	...
	private static CharPtr mkfuncname(LuaStateObject L, CharPtr modname) {
		CharPtr funcname;
	...
	private static int loader_C(LuaStateObject L) {
		CharPtr funcname;
		CharPtr name = LuaAuxLib.luaL_checkstring(L, 1);
	...
	private static int loader_Croot(LuaStateObject L) {
		CharPtr funcname;
		CharPtr filename;
		CharPtr name = LuaAuxLib.luaL_checkstring(L, 1);
	...
	private static int loader_preload(LuaStateObject L) {
		CharPtr name = LuaAuxLib.luaL_checkstring(L, 1);
	...
	public static int ll_require(LuaStateObject L) {
		CharPtr name = LuaAuxLib.luaL_checkstring(L, 1);
		int i;
		...
		for (i = 1; ; i++) {
			...
			if (Lua.lua_isnil(L, -1)) {
				LuaAuxLib.luaL_error(L, CharPtr.toCharPtr("module " + LuaConf.getLUA_QS() + " not found:%s"), name, Lua.lua_tostring(L, -2));
			}
...
//		 
	...
	private static void setfenv(LuaStateObject L) {
		LuaDebugState ar = new LuaDebugState();
		if (LuaDebug.lua_getstack(L, 1, ar) == 0 || LuaDebug.lua_getinfo(L, CharPtr.toCharPtr("f"), ar) == 0 || LuaAPI.lua_iscfunction(L, -1)) { // get calling function 
	...
	private static void dooptions(LuaStateObject L, int n) {
		int i;
		for (i = 2; i <= n; i++) {
	...
	private static void modinit(LuaStateObject L, CharPtr modname) {
		CharPtr dot;
		...
		LuaAPI.lua_pushlstring(L, modname, CharPtr.minus(dot, modname)); //(uint)
		LuaAPI.lua_setfield(L, -2, CharPtr.toCharPtr("_PACKAGE"));
	}

	private static int ll_module(LuaStateObject L) {
	...
	private static int ll_seeall(LuaStateObject L) {
	...
	private static void setpath(LuaStateObject L, CharPtr fieldname, CharPtr envname, CharPtr def) {
		...
		if (CharPtr.isEqual(path, null)) { // no environment variable? 
			...
			path = LuaAuxLib.luaL_gsub(L, path, CharPtr.toCharPtr(LuaConf.LUA_PATHSEP + LuaConf.LUA_PATHSEP), CharPtr.toCharPtr(LuaConf.LUA_PATHSEP + AUXMARK + LuaConf.LUA_PATHSEP));
	...
	private final static LuaLReg[] pk_funcs = { new LuaLReg(CharPtr.toCharPtr("loadlib"), new LuaLoadLib_delegate("ll_loadlib")), new LuaLReg(CharPtr.toCharPtr("seeall"), new LuaLoadLib_delegate("ll_seeall")), new LuaLReg(null, null) };

	private final static LuaLReg[] ll_funcs = { new LuaLReg(CharPtr.toCharPtr("module"), new LuaLoadLib_delegate("ll_module")), new LuaLReg(CharPtr.toCharPtr("require"), new LuaLoadLib_delegate("ll_require")), new LuaLReg(null, null) };

	public final static LuaCFunction[] loaders = { new LuaLoadLib_delegate("loader_preload"), new LuaLoadLib_delegate("loader_Lua"), new LuaLoadLib_delegate("loader_C"), new LuaLoadLib_delegate("loader_Croot"), null };

	public static class LuaLoadLib_delegate implements LuaCFunction {
		private String name;
		...
		public final int exec(LuaStateObject L) {
			...
			else if ((new String("ll_seeall")).equals(name)) {
				return ll_seeall(L);
			}
			else if ((new String("ll_module")).equals(name)) {
				return ll_module(L);
			}
			else if ((new String("ll_require")).equals(name)) {
				return ll_require(L);
			}
			else if ((new String("loader_preload")).equals(name)) {
				return loader_preload(L);
			}
			else if ((new String("loader_Lua")).equals(name)) {
				return loader_Lua(L);
			}
			else if ((new String("loader_C")).equals(name)) {
				return loader_C(L);
			}
			else if ((new String("loader_Croot")).equals(name)) {
				return loader_Croot(L);
			}
			else if ((new String("gctm")).equals(name)) {
				return gctm(L);
			}
			else {
				return 0;
	...
	public static int luaopen_package(LuaStateObject L) {
		int i;
