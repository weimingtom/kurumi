...
import com.iteye.weimingtom.kurumi.model.LoadState;
...
public class LuaUndump {
	...
	public static byte LoadByte(LoadState S) { //lu_byte
		return (byte)LoadChar(S); //lu_byte
	}
	...
	private static int LoadChar(LoadState S) {
		return ((Character)LoadVar(S, new ClassType(ClassType.TYPE_CHAR))).charValue();
	}

	private static int LoadInt(LoadState S) {
		int x = ((Integer)LoadVar(S, new ClassType(ClassType.TYPE_INT))).intValue();
		IF(x < 0, "bad integer");
		return x;
	}

	private static double LoadNumber(LoadState S) { //lua_Number
...
//		 
	public static Proto luaU_undump(LuaStateObject L, ZIO Z, Mbuffer buff, CharPtr name) {
