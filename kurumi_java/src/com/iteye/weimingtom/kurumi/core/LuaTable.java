...
import com.iteye.weimingtom.kurumi.model.TKey;
import com.iteye.weimingtom.kurumi.model.TKeyNk;
...
//		 
	...
	public static TKeyNk gkey(Node n) {
		return n.i_key.nk;
	}
...
//		 
	///#if LUAI_BITSINT > 26
	public static final int MAXBITS = 26; // in the dotnet port LUAI_BITSINT is 32 
	///#else
	...
	public static Node hashpow2(Table t, double n) { //lua_Number
...
//		 
	public static final int numints = ClassType.GetNumInts(); //const
...
//		 
	private static Node hashnum(Table t, double n) { //lua_Number
...
//		 
	private static int findindex(LuaStateObject L, Table t, TValue key) { //StkId
		int i;
	...
	public static int luaH_next(LuaStateObject L, Table t, TValue key) { //StkId
...
//		 
	...
	private static int computesizes(int[] nums, int[] narray) { //ref
		int i;
		int twotoi; // 2^i 
	...
	private static void setarrayvector(LuaStateObject L, Table t, int size) {
		int i;
	...
	private static void setnodevector(LuaStateObject L, Table t, int size) {
		int lsize;
		if (size == 0) {
	...
	private static void resize(LuaStateObject L, Table t, int nasize, int nhsize) {
		int i;
	...
	public static void luaH_resizearray(LuaStateObject L, Table t, int nasize) {
		int nsize = (Node.isEqual(t.node[0], dummynode)) ? 0 : LuaObject.sizenode(t);
		resize(L, t, nasize, nsize);
	}

	private static void rehash(LuaStateObject L, Table t, TValue ek) {
...
//		 
	...
	public static Table luaH_new(LuaStateObject L, int narray, int nhash) {
	...
	public static void luaH_free(LuaStateObject L, Table t) {
...
//		 
	private static TValue newkey(LuaStateObject L, Table t, TValue key) {
...
//		 
	public static TValue luaH_getnum(Table t, int key) {
		...
		if ((long)((key - 1) & 0xffffffff) < (long)(t.sizearray & 0xffffffff)) { //uint - uint
			...
			double nk = LuaLimits.cast_num(key); //lua_Number
			Node n = hashnum(t, nk);
			do {
				// check whether `key' is somewhere in the chain 
...
//		 
	...
	public static TValue luaH_set(LuaStateObject L, Table t, TValue key) {
	...
	public static TValue luaH_setnum(LuaStateObject L, Table t, int key) {
	...
	public static TValue luaH_setstr(LuaStateObject L, Table t, TString key) {
