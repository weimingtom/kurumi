...
public class LuacProgram {
	...
	private static Proto toproto(LuaStateObject L, int i) {
		return LuaObject.clvalue(TValue.plus(L.top, i)).l.p;
	}

	private static Proto combine(LuaStateObject L, int n) {
		if (n == 1) {
			return toproto(L, -1);
		}
		else {
	...
	private static int writer(LuaStateObject L, CharPtr p, int size, Object u) { //uint
		//UNUSED(L);
