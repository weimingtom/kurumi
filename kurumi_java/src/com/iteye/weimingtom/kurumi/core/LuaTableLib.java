...
public class LuaTableLib {
	private static int aux_getn(LuaStateObject L, int n) {
		LuaAuxLib.luaL_checktype(L, n, Lua.LUA_TTABLE);
		return LuaAuxLib.luaL_getn(L, n);
	}

	private static int foreachi(LuaStateObject L) {
		int i;
		int n = aux_getn(L, 1);
	...
	private static int _foreach(LuaStateObject L) {
	...
	private static int maxn(LuaStateObject L) {
		...
		while (LuaAPI.lua_next(L, 1) != 0) {
			...
			if (LuaAPI.lua_type(L, -1) == Lua.LUA_TNUMBER) {
				double v = LuaAPI.lua_tonumber(L, -1); //lua_Number
				if (v > max) {
					max = v;
				}
			}
	...
	private static int getn(LuaStateObject L) {
		LuaAPI.lua_pushinteger(L, aux_getn(L, 1));
		return 1;
	}

	private static int setn(LuaStateObject L) {
		LuaAuxLib.luaL_checktype(L, 1, Lua.LUA_TTABLE);
		///#ifndef luaL_setn
	...
	private static int tinsert(LuaStateObject L) {
	...
	private static int tremove(LuaStateObject L) {
		int e = aux_getn(L, 1);
	...
	private static void addfield(LuaStateObject L, LuaLBuffer b, int i) {
		LuaAPI.lua_rawgeti(L, 1, i);
		if (LuaAPI.lua_isstring(L, -1) == 0) {
			LuaAuxLib.luaL_error(L, CharPtr.toCharPtr("invalid value (%s) at index %d in table for " + LuaConf.LUA_QL("concat")), LuaAuxLib.luaL_typename(L, -1), i);
		}
		LuaAuxLib.luaL_addvalue(b);
	}


	private static int tconcat(LuaStateObject L) {
		LuaLBuffer b = new LuaLBuffer();
		int[] lsep = new int[1]; //uint
		int i, last;
		CharPtr sep = LuaAuxLib.luaL_optlstring(L, 2, CharPtr.toCharPtr(""), lsep); //out
		LuaAuxLib.luaL_checktype(L, 1, Lua.LUA_TTABLE);
...
//		 
	...
	private static void set2(LuaStateObject L, int i, int j) {
		LuaAPI.lua_rawseti(L, 1, i);
		LuaAPI.lua_rawseti(L, 1, j);
	}

	private static int sort_comp(LuaStateObject L, int a, int b) {
	...
	private static int auxsort_loop1(LuaStateObject L, int[] i) { //ref
		LuaAPI.lua_rawgeti(L, 1, ++i[0]);
		return sort_comp(L, -1, -2);
	}

	private static int auxsort_loop2(LuaStateObject L, int[] j) { //ref
		LuaAPI.lua_rawgeti(L, 1, --j[0]);
		return sort_comp(L, -3, -1);
	}

	private static void auxsort(LuaStateObject L, int l, int u) {
		while (l < u) {
			// for tail recursion 
			int i, j;
			...
			for (;;) {
				...
				while (true) {
					int[] i_ref = new int[1];
					i_ref[0] = i;
					int ret_1 = auxsort_loop1(L, i_ref); //ref
					i = i_ref[0];
					if (!(ret_1 != 0)) {
						break;
					}
				...
				while (true) {
					int[] j_ref = new int[1];
					j_ref[0] = i;
					int ret_2 = auxsort_loop2(L, j_ref); //ref
					j = j_ref[0];
					if (!(ret_2 != 0)) {
						break;
					}
	...
	private static int sort(LuaStateObject L) {
		int n = aux_getn(L, 1);
	...
	private final static LuaLReg[] tab_funcs = { new LuaLReg(CharPtr.toCharPtr("concat"), new LuaTableLib_delegate("tconcat")), new LuaLReg(CharPtr.toCharPtr("foreach"), new LuaTableLib_delegate("_foreach")), new LuaLReg(CharPtr.toCharPtr("foreachi"), new LuaTableLib_delegate("foreachi")), new LuaLReg(CharPtr.toCharPtr("getn"), new LuaTableLib_delegate("getn")), new LuaLReg(CharPtr.toCharPtr("maxn"), new LuaTableLib_delegate("maxn")), new LuaLReg(CharPtr.toCharPtr("insert"), new LuaTableLib_delegate("tinsert")), new LuaLReg(CharPtr.toCharPtr("remove"), new LuaTableLib_delegate("tremove")), new LuaLReg(CharPtr.toCharPtr("setn"), new LuaTableLib_delegate("setn")), new LuaLReg(CharPtr.toCharPtr("sort"), new LuaTableLib_delegate("sort")), new LuaLReg(null, null) };

	public static int luaopen_table(LuaStateObject L) {
		LuaAuxLib.luaL_register(L, CharPtr.toCharPtr(LuaLib.LUA_TABLIBNAME), tab_funcs);
		return 1;
	}

	public static class LuaTableLib_delegate implements LuaCFunction {
		private String name;
		...
		public final int exec(LuaStateObject L) {
			...
			else if ((new String("_foreach")).equals(name)) {
				return _foreach(L);
			}
			else if ((new String("foreachi")).equals(name)) {
				return foreachi(L);
			}
			else if ((new String("getn")).equals(name)) {
				return getn(L);
			}
			else if ((new String("maxn")).equals(name)) {
				return maxn(L);
			}
			else if ((new String("tinsert")).equals(name)) {
				return tinsert(L);
			}
			else if ((new String("tremove")).equals(name)) {
				return tremove(L);
			}
			else if ((new String("setn")).equals(name)) {
				return setn(L);
			}
			else if ((new String("sort")).equals(name)) {
				return sort(L);
			}
			else {
				return 0;
