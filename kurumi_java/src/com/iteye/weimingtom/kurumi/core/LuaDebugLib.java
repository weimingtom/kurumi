...
public class LuaDebugLib {
	private static int db_getregistry(LuaStateObject L) {
		LuaAPI.lua_pushvalue(L, Lua.LUA_REGISTRYINDEX);
		return 1;
	}

	private static int db_getmetatable(LuaStateObject L) {
	...
	private static int db_setmetatable(LuaStateObject L) {
	...
	private static int db_getfenv(LuaStateObject L) {
		LuaAPI.lua_getfenv(L, 1);
		return 1;
	}


	private static int db_setfenv(LuaStateObject L) {
	...
	private static void settabss(LuaStateObject L, CharPtr i, CharPtr v) {
		LuaAPI.lua_pushstring(L, v);
		LuaAPI.lua_setfield(L, -2, i);
	}

	private static void settabsi(LuaStateObject L, CharPtr i, int v) {
		LuaAPI.lua_pushinteger(L, v);
		LuaAPI.lua_setfield(L, -2, i);
	}

	private static LuaStateObject getthread(LuaStateObject L, int[] arg) { //out
	...
	private static void treatstackoption(LuaStateObject L, LuaStateObject L1, CharPtr fname) {
		if (L == L1) {
	...
	private static int db_getinfo(LuaStateObject L) {
		LuaDebugState ar = new LuaDebugState();
		int[] arg = new int[1];
		LuaStateObject L1 = getthread(L, arg); //out
	...
	private static int db_getlocal(LuaStateObject L) {
		int[] arg = new int[1];
		LuaStateObject L1 = getthread(L, arg); //out
		LuaDebugState ar = new LuaDebugState();
		CharPtr name;
	...
	private static int db_setlocal(LuaStateObject L) {
		int[] arg = new int[1];
		LuaStateObject L1 = getthread(L, arg); //out
		LuaDebugState ar = new LuaDebugState();
		if (LuaDebug.lua_getstack(L1, LuaAuxLib.luaL_checkint(L, arg[0] + 1), ar) == 0) { // out of range? 
	...
	private static int auxupvalue(LuaStateObject L, int get) {
		CharPtr name;
	...
	private static int db_getupvalue(LuaStateObject L) {
		return auxupvalue(L, 1);
	}

	private static int db_setupvalue(LuaStateObject L) {
	...
	private static void hookf(LuaStateObject L, LuaDebugState ar) {
	...
	public static class hookf_delegate implements LuaHook {
		public final void exec(LuaStateObject L, LuaDebugState ar) {
			hookf(L, ar);
		}
	}
	...
	private static void gethooktable(LuaStateObject L) {
		LuaAPI.lua_pushlightuserdata(L, KEY_HOOK);
		LuaAPI.lua_rawget(L, Lua.LUA_REGISTRYINDEX);
	...
	private static int db_sethook(LuaStateObject L) {
		int[] arg = new int[1];
		int mask, count;
		LuaHook func;
		LuaStateObject L1 = getthread(L, arg); //out
	...
	private static int db_gethook(LuaStateObject L) {
		int[] arg = new int[1];
		LuaStateObject L1 = getthread(L, arg); //out
		CharPtr buff = CharPtr.toCharPtr(new char[5]);
		int mask = LuaDebug.lua_gethookmask(L1);
		LuaHook hook = LuaDebug.lua_gethook(L1);
	...
	private static int db_debug(LuaStateObject L) {
		for (;;) {
	...
	private static int db_errorfb(LuaStateObject L) {
		int level;
		...
		while (LuaDebug.lua_getstack(L1, level++, ar) != 0) {
			...
			if (CharPtr.isNotEqualChar(ar.namewhat, '\0')) { // is there a name? 
				...
				if (CharPtr.isEqualChar(ar.what, 'm')) { // main? 
					...
					LuaAPI.lua_pushfstring(L, CharPtr.toCharPtr(" in function <%s:%d>"), ar.short_src, ar.linedefined);
				}
			}
	...
	private final static LuaLReg[] dblib = { new LuaLReg(CharPtr.toCharPtr("debug"), new LuaDebugLib_delegate("db_debug")), new LuaLReg(CharPtr.toCharPtr("getfenv"), new LuaDebugLib_delegate("db_getfenv")), new LuaLReg(CharPtr.toCharPtr("gethook"), new LuaDebugLib_delegate("db_gethook")), new LuaLReg(CharPtr.toCharPtr("getinfo"), new LuaDebugLib_delegate("db_getinfo")), new LuaLReg(CharPtr.toCharPtr("getlocal"), new LuaDebugLib_delegate("db_getlocal")), new LuaLReg(CharPtr.toCharPtr("getregistry"), new LuaDebugLib_delegate("db_getregistry")), new LuaLReg(CharPtr.toCharPtr("getmetatable"), new LuaDebugLib_delegate("db_getmetatable")), new LuaLReg(CharPtr.toCharPtr("getupvalue"), new LuaDebugLib_delegate("db_getupvalue")), new LuaLReg(CharPtr.toCharPtr("setfenv"), new LuaDebugLib_delegate("db_setfenv")), new LuaLReg(CharPtr.toCharPtr("sethook"), new LuaDebugLib_delegate("db_sethook")), new LuaLReg(CharPtr.toCharPtr("setlocal"), new LuaDebugLib_delegate("db_setlocal")), new LuaLReg(CharPtr.toCharPtr("setmetatable"), new LuaDebugLib_delegate("db_setmetatable")), new LuaLReg(CharPtr.toCharPtr("setupvalue"), new LuaDebugLib_delegate("db_setupvalue")), new LuaLReg(CharPtr.toCharPtr("traceback"), new LuaDebugLib_delegate("db_errorfb")), new LuaLReg(null, null) };

	public static class LuaDebugLib_delegate implements LuaCFunction {
		private String name;
	...
	public static int luaopen_debug(LuaStateObject L) {
