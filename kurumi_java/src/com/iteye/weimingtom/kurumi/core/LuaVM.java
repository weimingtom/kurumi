...
public class LuaVM {
	public static int tostring(LuaStateObject L, TValue o) { //StkId
		return ((LuaObject.ttype(o) == Lua.LUA_TSTRING) || (luaV_tostring(L, o) != 0)) ? 1 : 0;
	}

	public static int tonumber(TValue[] o, TValue n) { //StkId - ref
		return ((LuaObject.ttype(o[0]) == Lua.LUA_TNUMBER || (((o[0]) = luaV_tonumber(o[0], n)) != null))) ? 1 : 0;
	}

	public static int equalobj(LuaStateObject L, TValue o1, TValue o2) {
	...
	public static TValue luaV_tonumber(TValue obj, TValue n) {
		double[] num = new double[1]; //lua_Number
		if (LuaObject.ttisnumber(obj)) {
			return obj;
		}
		if (LuaObject.ttisstring(obj) && (LuaObject.luaO_str2d(LuaObject.svalue(obj), num) != 0)) { //out
	...
	public static int luaV_tostring(LuaStateObject L, TValue obj) { //StkId
		if (!LuaObject.ttisnumber(obj)) {
			return 0;
		}
		else {
	...
	private static void traceexec(LuaStateObject L, InstructionPtr pc) {
		byte mask = L.hookmask; //lu_byte
...
//			   or when enter a new line 
	...
	private static void callTMres(LuaStateObject L, TValue res, TValue f, TValue p1, TValue p2) { //StkId
		int result = LuaDo.savestack(L, res); //ptrdiff_t - Int32
...
TValue.dec(top); //ref
	...
	private static void callTM(LuaStateObject L, TValue f, TValue p1, TValue p2, TValue p3) {
		LuaObject.setobj2s(L, L.top, f); // push function 
		LuaObject.setobj2s(L, TValue.plus(L.top, 1), p1); // 1st argument 
		LuaObject.setobj2s(L, TValue.plus(L.top, 2), p2); // 2nd argument 
	...
	public static void luaV_gettable(LuaStateObject L, TValue t, TValue key, TValue val) { //StkId
		int loop;
	...
	public static void luaV_settable(LuaStateObject L, TValue t, TValue key, TValue val) { //StkId
		int loop;
	...
	private static int call_binTM(LuaStateObject L, TValue p1, TValue p2, TValue res, TMS event_) { //StkId
	...
	private static TValue get_compTM(LuaStateObject L, Table mt1, Table mt2, TMS event_) {
	...
	private static int call_orderTM(LuaStateObject L, TValue p1, TValue p2, TMS event_) {
	...
	private static int l_strcmp(TString ls, TString rs) {
		...
		for (;;) {
			...
			if (temp != 0) {
				...
				int len = l.toString().length(); // index of first `\0' in both strings  - (uint) - uint
	...
	public static int luaV_lessthan(LuaStateObject L, TValue l, TValue r) {
		int res;
	...
	private static int lessequal(LuaStateObject L, TValue l, TValue r) {
		int res;
		if (LuaObject.ttype(l) != LuaObject.ttype(r)) {
			return LuaDebug.luaG_ordererror(L, l, r);
		}
	...
	private static CharPtr mybuff = null;

	public static int luaV_equalval(LuaStateObject L, TValue t1, TValue t2) {
		TValue tm = null;
	...
	public static void luaV_concat(LuaStateObject L, int total, int last) {
		do {
			...
			if (!(LuaObject.ttisstring(TValue.minus(top, 2)) || LuaObject.ttisnumber(TValue.minus(top, 2))) || (tostring(L, TValue.minus(top, 1)) == 0)) {
				...
				int tl = LuaObject.tsvalue(TValue.minus(top, 1)).len; //uint
				CharPtr buffer;
				int i;
				// collect total length 
				for (n = 1; n < total && (tostring(L, TValue.minus(TValue.minus(top, n), 1)) != 0); n++) { //FIXME:
					int l = LuaObject.tsvalue(TValue.minus(TValue.minus(top, n), 1)).len; //uint
				...
				for (i = n; i > 0; i--) {
					// concat all strings 
					int l = LuaObject.tsvalue(TValue.minus(top, i)).len; //uint
					LuaConf.memcpy_char(buffer.chars, tl, LuaObject.svalue(TValue.minus(top, i)).chars, (int)l); //(int)
					tl += l;
				}
	...
	public static void Arith(LuaStateObject L, TValue ra, TValue rb, TValue rc, TMS op) { //StkId
...
//		 
	public static void runtime_check(LuaStateObject L, boolean c) {
		ClassType.Assert(c);
	}
	...
	public static TValue RA(LuaStateObject L, TValue base_, long i) { //Instruction - UInt32 - StkId
		return TValue.plus(base_, LuaOpCodes.GETARG_A(i));
	}

	public static TValue RB(LuaStateObject L, TValue base_, long i) { //Instruction - UInt32 - StkId
		return TValue.plus(base_, LuaOpCodes.GETARG_B(i));
	}

	public static TValue RC(LuaStateObject L, TValue base_, long i) { //Instruction - UInt32 - StkId
		return TValue.plus(base_, LuaOpCodes.GETARG_C(i));
	}

	public static TValue RKB(LuaStateObject L, TValue base_, long i, TValue[] k) { //Instruction - UInt32 - StkId
	...
	public static TValue RKC(LuaStateObject L, TValue base_, long i, TValue[] k) { //Instruction - UInt32 - StkId
	...
	public static TValue KBx(LuaStateObject L, long i, TValue[] k) { //Instruction - UInt32
		return k[LuaOpCodes.GETARG_Bx(i)];
	}

	public static void dojump(LuaStateObject L, InstructionPtr pc, int i) {
		pc.pc += i;
	...
	public static void arith_op(LuaStateObject L, OpDelegate op, TMS tm, TValue base_, long i, TValue[] k, TValue ra, InstructionPtr pc) { //StkId - Instruction - UInt32 - StkId
	...
	private static void Dump(int pc, long i) { //Instruction - UInt32
	...
	public static void luaV_execute(LuaStateObject L, int nexeccalls) {
		LClosure cl;
		TValue base_; //StkId
		TValue[] k;
		//const
 InstructionPtr pc;
		...
		while (true) {
			...
			for (;;) {
				...
				pc_ref[0] = pc;
				InstructionPtr ret = InstructionPtr.inc(pc_ref); //ref
				pc = pc_ref[0];
				//const
				...
				switch (LuaOpCodes.GET_OPCODE(i)) {
					...
					case OP_LOADBOOL: {
							...
							if (LuaOpCodes.GETARG_C(i) != 0) {
								...
								pc_ref2[0] = pc;
								InstructionPtr.inc(pc_ref2); // skip next instruction (if C)  - ref
								pc = pc_ref2[0];
							}
							continue;
						}
					case OP_LOADNIL: {
							...
							do {
								...
								rb_ref[0] = rb;
								TValue ret2 = TValue.dec(rb_ref); //ref - StkId
								rb = rb_ref[0];
...
TValue rb = RB(L, base_, i);
					...
					case OP_EQ: {
							...
							InstructionPtr[] pc_ref2 = new InstructionPtr[1];
							pc_ref2[0] = pc;
							InstructionPtr.inc(pc_ref2); //ref
							pc = pc_ref2[0];
							continue;
						}
					case OP_LT: {
							...
							pc_ref3[0] = pc;
							InstructionPtr.inc(pc_ref3); //ref
							pc = pc_ref3[0];
							continue;
						}
					case OP_LE: {
							...
							pc_ref4[0] = pc;
							InstructionPtr.inc(pc_ref4); //ref
							pc = pc_ref4[0];
							continue;
						}
					case OP_TEST: {
							...
							pc_ref5[0] = pc;
							InstructionPtr.inc(pc_ref5); //ref
							pc = pc_ref5[0];
							continue;
						}
					case OP_TESTSET: {
							...
							pc_ref6[0] = pc;
							InstructionPtr.inc(pc_ref6); //ref
							pc = pc_ref6[0];
							continue;
						}
					case OP_CALL: {
					...
					case OP_FORLOOP: {
							double step = LuaObject.nvalue(TValue.plus(ra, 2)); //lua_Number
							double idx = LuaConf.luai_numadd(LuaObject.nvalue(ra), step); // increment index  - lua_Number
							double limit = LuaObject.nvalue(TValue.plus(ra, 1)); //lua_Number
					...
					case OP_FORPREP: {
							...
							if (retxxx == 0) {
								...
								plimit_ref[0] = plimit;
								retxxx = tonumber(plimit_ref, TValue.plus(ra, 1)); //ref
								plimit = plimit_ref[0];
								if (retxxx == 0) {
									...
									pstep_ref[0] = pstep;
									retxxx = tonumber(pstep_ref, TValue.plus(ra, 2)); //ref
									pstep = pstep_ref[0];
									if (retxxx == 0) {
					...
					case OP_TFORLOOP: {
							TValue cb = TValue.plus(ra, 3); // call base  - StkId
